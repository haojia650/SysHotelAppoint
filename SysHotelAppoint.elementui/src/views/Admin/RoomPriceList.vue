...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改房间价格' : '添加房间价格'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="价格" prop="Price">
                            <el-input-number  v-model="formData.Price"  placeholder="请输入价格"     :clearable="true"  :min="1" :max="1000000"></el-input-number>
                        </el-form-item>
                    </el-col>
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/RoomPrice/List" :column="columnList" :where="where">
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/RoomPrice/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/RoomPrice/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/RoomPrice/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/RoomPrice/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
