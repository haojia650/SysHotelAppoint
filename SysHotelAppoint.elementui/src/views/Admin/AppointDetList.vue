            ...
            <div class="margin-top-sm">
                <el-form :inline="true" :model="searchForm" size="default">
                    ...
                    <el-form-item label="年龄" prop="Age">
                        <el-input v-model.trim="searchForm.Age"  placeholder="请输入年龄"  :clearable="true" ></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="预约订单">
                        <SigleSelect url="/Appoint/List" class="search-input" columnName="No" :clearable="true" columnValue="Id"
...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改预约明细' : '添加预约明细'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
            <el-form v-if="editorShow" ref="editModalForm" :rules="editModalFormRules" :model="formData" label-width="140px"
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="预约订单" prop="AppointId">
                    ...
                    <el-col :span="24">
                        <el-form-item label="年龄" prop="Age">
                            <el-input type="text" v-model="formData.Age"  placeholder="请输入年龄"     :clearable="true"></el-input>
                        </el-form-item>
                    </el-col>
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/AppointDet/List" :column="columnList" :where="where">
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/AppointDet/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/AppointDet/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/AppointDet/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/AppointDet/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
