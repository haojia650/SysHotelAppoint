            ...
            <div class="margin-top-sm">
                <el-form :inline="true" :model="searchForm" size="default">
                    ...
                    <el-form-item label="分类" prop="GroupName">
                        <el-input v-model.trim="searchForm.GroupName"  placeholder="请输入分类"  :clearable="true" ></el-input>
                    </el-form-item>
 							 <el-form-item label="是否免费" class="search-input">
                 		 <el-select v-model="searchForm.IsFree" class="search-input" :clearable="true" placeholder="请选择是否免费" >
...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改设备信息' : '添加设备信息'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="是否免费" prop="IsFree">
                       		  <el-switch v-model="formData.IsFree"  :active-value="true" :inactive-value="false">
                            </el-switch>                          
                        </el-form-item>
                    </el-col>
                    ...
                    <el-col :span="24">
                        <el-form-item label="显示顺序" prop="Sort">
                            <el-input-number  v-model="formData.Sort"  placeholder="请输入显示顺序"     :clearable="true"  :min="1" :max="1000000"></el-input-number>
                        </el-form-item>
                    </el-col>

                    <el-col :span="24">
                        <el-form-item label="分类" prop="GroupName">
                            <el-input type="text" v-model="formData.GroupName"  placeholder="请输入分类"     :clearable="true"></el-input>
                        </el-form-item>
                    </el-col>
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/EquipmentInfo/List" :column="columnList" :where="where">
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/EquipmentInfo/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/EquipmentInfo/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/EquipmentInfo/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/EquipmentInfo/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
